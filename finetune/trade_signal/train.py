"""
Fine-tune DistilBERT for Trade Signal Classification.

Classifies FinColl predictions into trading actions:
- STRONG_LONG, LONG, HOLD, SHORT, STRONG_SHORT, AVOID

Usage:
    python generate_training_data.py --output data --count 200
    python train.py --data data --output models/trade_signal
"""
...
class TradeSignalDataset(Dataset):
    """PyTorch dataset for trade signal classification."""
...
def train(
    ...
    """Fine-tune DistilBERT on trade signal classification."""
    ...
    train_dataset = TradeSignalDataset(data_dir / "train.jsonl", tokenizer, max_length)
    val_dataset = TradeSignalDataset(data_dir / "val.jsonl", tokenizer, max_length)
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=50,
        report_to="none",
        save_total_limit=1,  # Only keep best checkpoint to save space
    )
...
def export_to_onnx(model, tokenizer, output_dir: Path, max_length: int = 128):
    ...
    try:
        ...
        dummy_input = tokenizer(
            "FinColl predicts bullish with 80% confidence",
            return_tensors="pt",
            padding="max_length",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="models/trade_signal")
